...
struct RuleEditorView: View {
    ...
    /// Last name we auto-generated for this rule. Used to detect "user
    /// hasn't manually typed their own name yet" so auto-naming can keep
    /// refining as hosts/target change — and steps out of the way the
    /// moment they do type something.
    ...
    var body: some View {
        ...
        .onChange(of: rule.target.browserBundleID) { _, _ in
            refreshProfiles()
            refreshAutoName()
        }
        .onChange(of: rule.match) { _, _ in refreshAutoName() }
    }

    /// Update the rule's name from its hosts + target — but only while
    /// the user hasn't typed a custom one. `Rule.nameIsAutoGenerated`
    /// is the gate: we never overwrite something the user authored.
    private func refreshAutoName() {
        guard rule.nameIsAutoGenerated(lastAutoName: lastAutoName) else { return }
        guard let candidate = rule.autoGeneratedName() else { return }
        ...
        WrappingHStack(spacing: 6, lineSpacing: 6) {
            ...
            AddChipField(
                ...
                onCommit: { committed in
                    ...
                    // the v1 bug here.
                    guard let host = HostChipMatcher.normalizeChip(from: committed) else { return }
                    var newHosts = chips.hosts
    ...
    private var matcherSummary: String {
        ...
        case .urlPrefix(let v):   return "urlPrefix = \(v)"
